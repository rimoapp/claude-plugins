...
# Get the default branch name (main/master or configured remote HEAD).
# Arguments: $1 = directory path
# Outputs: branch name on stdout
# Returns: 0 on success, 1 if the default branch cannot be determined.
get_default_branch() {
  local dir="$1"
...
is_on_default_branch() {
  ...
  current="$(git -C "$dir" rev-parse --abbrev-ref HEAD 2>/dev/null)" || return 1
  default_branch="$(get_default_branch "$dir")" || return 1
